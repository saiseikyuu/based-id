...
export async function GET(
  ...
  // Accept pre-fetched data as query params (fastest path)
  // or fall back to calling our own Node.js API
  let totalXp  = Number(searchParams.get("xp")  ?? 0);
  let streak   = Number(searchParams.get("s")    ?? 0);
  let repScore = Number(searchParams.get("rep")  ?? 0);

  if (!searchParams.has("xp")) {
    try {
      const siteUrl = process.env.NEXT_PUBLIC_SITE_URL ?? "https://basedid.space";
      const res = await fetch(`${siteUrl}/api/hunters/rank?wallet=${wallet.toLowerCase()}`, {
        headers: { "x-internal": "1" },
      });
      if (res.ok) {
        const data = await res.json() as { totalXp?: number; breakdown?: { checkinXp?: number } };
