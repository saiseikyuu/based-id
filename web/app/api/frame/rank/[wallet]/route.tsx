...
function renderCard(totalXp: number, streak: number, repScore: number, walletShort: string) {
  ...
  return (
    <div style={{
      ...
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: "4px", background: rankColor, borderRadius: "0 0 2px 2px" }} />
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "40px" }}>
        ...
        <div style={{ fontSize: "13px", color: "rgba(255,255,255,0.25)", fontFamily: "monospace" }}>{walletShort}</div>
      </div>
          ...
          <div style={{ display: "flex", gap: "32px" }}>
            {[
              ...
              <div key={label} style={{ display: "flex", flexDirection: "column" }}>
                ...
                <div style={{ fontSize: "11px", color: "rgba(255,255,255,0.25)", textTransform: "uppercase", letterSpacing: "0.15em" }}>{label}</div>
              </div>
            ))}
          </div>
        </div>
...
export async function GET(
  ...
  // Fetch from Supabase REST API directly (avoids client library issues in edge)
  ...
  return new ImageResponse(renderCard(totalXp, streak, repScore, short), {
    width: 1200,
    height: 630,
  });
}
