...
export function DropEntry({
  ...
  // Read balanceOf directly onchain
  ...
  // Load already-completed tasks (entry status + Twitter verifications)
  const loadState = useCallback(async () => {
    ...
    // 2. Check Twitter verifications for any follow_x tasks
  ...
  }, [address, drop?.id, tasks]);

  useEffect(() => { loadState(); }, [loadState]);

  // Handle Twitter OAuth callback params (?twitter_ok=handle or ?twitter_error=...)
  useEffect(() => {
  ...
  function handleTwitterOAuth(task: Task) {
    ...
    const url = `/api/twitter/auth?wallet=${address}&handle=${handle}&drop_id=${drop.id}&task_id=${task.id}`;
  ...
  function getTaskAction(task: Task): { label: string; url?: string; onchain: boolean; twitter: boolean } {
    switch (task.type) {
      ...
      case "join_discord":
        return { label: "Join Discord", url: task.params.invite as string ?? "#", onchain: false, twitter: false };
      case "hold_nft":
      case "hold_based_id":
  ...
  return (
    <div className="rounded-2xl border border-white/[0.07] bg-white/[0.02] p-6 space-y-6">
      <div className="space-y-1">
        ...
        <p className="text-zinc-500 text-xs">Complete all tasks to enter.</p>
      </div>
              ...
              <div key={task.id} className={`flex items-center justify-between gap-3 rounded-xl border px-4 py-3 transition-colors ${
                ...
                {!done && (
                  <div className="flex items-center gap-2 flex-shrink-0">
                    {/* Twitter OAuth button — primary if enabled, fallback to self-attest */}
                    {action.twitter && TWITTER_ENABLED ? (
                      ...
                      // Self-attest fallback when Twitter OAuth not configured
                      <button
      ...
      <button
        ...
        className="w-full py-4 rounded-xl font-bold text-sm transition-all bg-white text-black hover:bg-zinc-100 disabled:opacity-30 disabled:cursor-not-allowed shadow-[0_2px_40px_rgba(255,255,255,0.07)]"
      >
