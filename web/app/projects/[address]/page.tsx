  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="relative flex-1">
        <div className="absolute inset-0 pointer-events-none" style={{ background: "radial-gradient(ellipse 50% 30% at 50% 0%, rgba(37,99,235,0.06), transparent 60%)" }} />

        <div className="relative max-w-5xl mx-auto px-6 pt-10 pb-20 space-y-10">

          {/* Breadcrumb */}
          <div className="flex items-center gap-2 text-[11px] text-zinc-600">
            <Link href="/projects" className="hover:text-zinc-400 transition-colors">Projects</Link>
            <span>/</span>
            <span className="text-zinc-400 truncate">{project.name}</span>
          </div>

          {/* Project header */}
          <div className="flex items-start gap-6 flex-wrap">
            {/* Logo */}
            <div className="w-20 h-20 rounded-2xl overflow-hidden border border-white/[0.08] flex-shrink-0">
              {project.logo_url ? (
                // eslint-disable-next-line @next/next/no-img-element
            ...
            <div className="flex-1 min-w-0 space-y-3">
              <div className="flex items-center gap-3 flex-wrap">
                <h1 className="text-white font-black text-3xl" style={DISPLAY}>{project.name}</h1>
              ...
              <div className="flex items-center gap-4 flex-wrap">
                {project.twitter && (
                  <a href={`https://x.com/${project.twitter.replace("@", "")}`} target="_blank" rel="noopener noreferrer"
                    ...
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.744l7.737-8.835L1.254 2.25H8.08l4.259 5.631 5.905-5.631zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                ...
                <span className="text-zinc-700 text-[10px] font-mono">{project.address.slice(0, 6)}…{project.address.slice(-4)}</span>
              </div>
            </div>
          </div>
      ...
      <footer className="border-t border-white/[0.04] px-6 py-5">
        <div className="max-w-5xl mx-auto flex items-center justify-between gap-4 flex-wrap">
          ...
          <Link href="/projects" className="text-zinc-600 text-[11px] hover:text-zinc-400 transition-colors">← All projects</Link>
        </div>
      </footer>
    </div>
  );
